...
def evaluation(model, data_loader, device, config):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_output = model.text_encoder(text_input.input_ids, attention_mask = text_input.attention_mask, mode='text',CP_U = model.CP_U, CP_V = model.CP_V, CP_C = model.CP_C)  
    ...
    for video, video_id in data_loader: 
        ...
        video_feat = model.visual_encoder(video, CP_U = model.CP_U, CP_V = model.CP_V, CP_C = model.CP_C) # (8 * 8, 577, 768)      
        video_embed = model.vision_proj(video_feat[:,0,:])   
        video_embed = video_embed.view(B,N,-1).mean(dim=1)
        video_embed = F.normalize(video_embed,dim=-1)  # (8 * 764，batch_size, seq_len)
       
        video_feat = video_feat.view(B,-1,video_feat.shape[-1]) # (bs, 8*577, hidden_dim )
    ...
    sims_matrix = video_embeds @ text_embeds.t() # (1000 * 1000)
    ...
    for i,sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)): 
        ...
        encoder_output = video_feats[start+i].repeat(config['k_test'],1,1).to(device,non_blocking=True) # (k, 577*8, 768)
...
def main(args, config):
    ...
    for epoch in range(0, config['max_epoch']):    
        ...
        if args.evaluate:  
            ...
            if utils.is_main_process():  
                test_result = itm_eval(score_test_i2t, score_test_t2i, test_loader.dataset.txt2video, test_loader.dataset.video2txt)     
                print(test_result)
                save_obj = {
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
