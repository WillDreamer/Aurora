...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        original_shape = image.shape  # (B, T, C, H, W)
        image = image.reshape(image.size(0) * image.size(1), 3, self.image_size, self.image_size) # (B*T, C, H, W)
        ...
        text = self.tokenizer(caption, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(image.device) # (B,seq_len)
        ...
        if momentum:
            with torch.no_grad():
                ...
                image_embeds_m = self.visual_encoder_m(image, CP_U = self.CP_U, CP_V = self.CP_V, CP_C = self.CP_C) 
                ...
                text_output_m = self.text_encoder_m(text.input_ids, attention_mask = text.attention_mask,                      
                                                    return_dict = True, mode = 'text',  CP_U = self.CP_U, CP_V = self.CP_V, CP_C = self.CP_C)    
        ...
        loss_ita = (loss_i2t+loss_t2i)/2  # ITC loss
